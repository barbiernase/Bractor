...
const SZ={HW:214,HHEAD:24,ROW:18,HPAD:8,GAPH:12,APAD:14,AHEAD:26,CPAD:18,CHEAD:32,GAPA:14,GAPC:44,LANEH:66,LHEAD:32,LROW:30,LANEGAP:90,LANEW:186,LANEGX:16,MAXROW:3000};
let board={};
...
function laneH(n){ return n.kind==='process' ? SZ.LHEAD + Math.max(1,(n.process.rules||[]).length)*SZ.LROW : SZ.LANEH; }
function placeLane(items,y,rightBound){ let x=0,yy=y,rowMax=0;
  items.forEach(n=>{ const h=laneH(n);
    if(x>0&&x+SZ.LANEW>rightBound){x=0;yy+=rowMax+SZ.LANEGX;rowMax=0;}
    board.lane[n.id]={x,y:yy,w:SZ.LANEW,h,node:n}; board.pick.push({x,y:yy,w:SZ.LANEW,h,node:n});
    rowMax=Math.max(rowMax,h); x+=SZ.LANEW+SZ.LANEGX; }); }
...
function render(){
  ...
  Object.values(board.lane).forEach(b=>{ const n=b.node; const [x,y]=S(b.x,b.y); const act=simActive.has(n.id); const col=KIND_COL[n.kind];
    ...
    if(n.kind==='process'){
      ...
      // Transitionen: Auf <Events> → Sende <Command> (+ Join / Fan-out / Kompensation)
      const maxW=(b.w-22)*z;
      (n.process.rules||[]).forEach((r,i)=>{ const ry=y+(SZ.LHEAD+i*SZ.LROW)*z;
        ctx.font=`${8.7*z}px -apple-system,sans-serif`;
        const join=r.join==='count'?' (alle '+(r.sammel||'')+')':'';
        ctx.fillStyle='#9cc2ff';
        ctx.fillText(clip('Auf '+(r.when||[]).join(' + ')+join, maxW), x+12*z, ry+5*z);
        ctx.fillStyle='#f0b464';
        const send='→ '+(r.fanOut?'je ':'')+r.sends+(r.compensates?'   ↩ '+r.compensates:'');
        ctx.fillText(clip(send, maxW), x+18*z, ry+18*z);
      });
    } else {
