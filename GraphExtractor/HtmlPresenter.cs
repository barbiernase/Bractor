...
#de .gnode2.n-aggregate{border-color:#7c5cff}
...
(function(){
  ...
  function neuerKnoten(kind,x,y){
    ...
    if(kind==="aggregate")MODEL.aggregate.push({name:uniq("NeuesAggregat"),namespace:defaultNs(),state:[],decider:[],applier:[],...pos});
  ...
  function renderGraph(){
    ...
    root.append(h("div",{class:"glegend"},
      ...
      h("span",{},"· Doppelklick auf die Fläche = Rohzelle · Kopf ziehen rastet ein · Port ziehen = verbinden")));
    autoLayout();
  ...
  function showPicker(canvas,x,y){
    ...
    pick.append(h("div",{class:"gpick-t"},"Was soll hier entstehen?"),
      ...
      opt("valueobject","Value Object"),opt("enum","Enum"),opt("aggregate","Aggregat"),opt("saga","Saga"));
    canvas.append(pick);
  ...
  function nodeEditor(n){
    const el=h("div",{class:"gnode2 n-"+n.kind+(["aggregate","saga"].includes(n.kind)?" wide":"")});
    el.dataset.id=n.id;
    ...
    if(n.kind==="aggregate")body.append(aggCard(n.ref,MODEL.aggregate.indexOf(n.ref)));
