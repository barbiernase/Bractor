...
(function(){
  ...
  // NUR der Node-Editor: links die Fläche, rechts der Inspektor des gewählten Knotens.
  let SELECTED=null;
  function render(){ renderGraph(); if(SELECTED&&findNode(SELECTED)?.ref) renderInspector(SELECTED); else { SELECTED=null; inspectorHint(); } }

  function findNode(id){
    const i=id.indexOf(":"), pfx=id.slice(0,i), name=id.slice(i+1);
    if(pfx==="rec"){const r=MODEL.records.find(r=>r.name===name);return r?{ref:r,kind:r.kind,name}:null;}
    if(pfx==="agg"){const a=MODEL.aggregate.find(a=>a.name===name);return a?{ref:a,kind:"aggregate",name}:null;}
    if(pfx==="enum"){const e=MODEL.enums.find(e=>e.name===name);return e?{ref:e,kind:"enum",name}:null;}
    if(pfx==="saga"){const s=MODEL.sagas.find(s=>s.name===name);return s?{ref:s,kind:"saga",name}:null;}
    return null;
  }
  function inspectorHint(){document.getElementById("de-out").innerHTML='<div class="hint">Einen Knoten anklicken, um ihn zu bearbeiten. Neue Knoten oben in der Werkzeugleiste anlegen. Vom Port ziehen = verbinden.</div>';}
  function renderInspector(id){
    const out=document.getElementById("de-out");out.innerHTML="";
    const nd=findNode(id);if(!nd||!nd.ref){inspectorHint();return;}
    out.append(h("div",{class:"sectitle"},"Inspektor · "+(nd.kind==="aggregate"?"Aggregat":nd.kind==="enum"?"Enum":nd.kind==="saga"?"Saga":kindLabel(nd.kind))));
    if(nd.kind==="aggregate")out.append(aggCard(nd.ref,MODEL.aggregate.indexOf(nd.ref)));
    else if(nd.kind==="enum")out.append(enumCard(nd.ref,MODEL.enums.indexOf(nd.ref)));
    else if(nd.kind==="saga")out.append(sagaCard(nd.ref,MODEL.sagas.indexOf(nd.ref)));
    else out.append(recordCard(nd.ref,MODEL.records.indexOf(nd.ref)));
  }
  function enumCard(e,ei){const c=h("div",{class:"card"});
    c.append(h("h3",{},h("span",{class:"k vo"},"Enum"),inp(e.name,v=>e.name=v,"Enum"),
      ...
      h("button",{class:"rm",onclick:()=>{MODEL.enums.splice(ei,1);SELECTED=null;render();}},"✕")));
  ...
  function addRecord(kind){
    ...
    const name="Neu"+kindLabel(kind).replace(/\s/g,"");
    MODEL.records.push({name,kind,namespace:defaultNs(),felder});
    SELECTED="rec:"+name;render();
  }
  function addAggregat(){const name="NeuesAggregat";MODEL.aggregate.push({name,namespace:defaultNs(),state:[],decider:[],applier:[]});SELECTED="agg:"+name;render();}
  function addEnum(){const name="NeuEnum";MODEL.enums.push({name,namespace:defaultNs(),werte:["A","B"]});SELECTED="enum:"+name;render();}
  function addSaga(){const name="NeuerProzess";MODEL.sagas.push({name,namespace:defaultNs(),triggerEvent:"",schritte:[],extraUsings:[]});SELECTED="saga:"+name;render();}
  ...
  function selectNode(id){SELECTED=id;
    document.querySelectorAll("#de-form .gnode.sel").forEach(n=>n.classList.remove("sel"));
    const el=[...document.querySelectorAll("#de-form .gnode")].find(x=>x.dataset.id===id);if(el)el.classList.add("sel");
    renderInspector(id);}
  function graphEdges(){
    const es=[];
  ...
  function autoLayout(){
    // Spalten wie Event Modeling; y stapelt PRO Spalte (kein Überlappen).
    const spalte={command:40,aggregate:400,event:760,rejection:760,valueobject:1080,enum:1080,saga:400};
    const yByX={};
    graphNodes().forEach(n=>{const r=n.ref;const x=spalte[n.kind]!==undefined?spalte[n.kind]:40;
      if(typeof r.x==="number"&&typeof r.y==="number"){yByX[x]=Math.max(yByX[x]||40,r.y+80);return;}
      const y=yByX[x]||40; r.x=x; r.y=y; yByX[x]=y+80;});
  }
  function renderGraph(){
    ...
    root.append(h("div",{class:"glegend"},
      ...
      h("span",{},"· Knoten ziehen rastet ein · Port ziehen = verbinden · Klick = bearbeiten")));
    if(!graphNodes().length){root.append(h("div",{class:"hint"},"Leer — oben in der Werkzeugleiste einen Knoten anlegen."));return;}
    autoLayout();
  ...
  function nodeEl(n){
    const el=h("div",{class:"gnode n-"+n.kind+(n.id===SELECTED?" sel":"")});
    el.dataset.id=n.id;
    el.style.left=(n.ref.x||0)+"px";el.style.top=(n.ref.y||0)+"px";
    const label=n.kind==="aggregate"?"Aggregat":n.kind==="enum"?"Enum":n.kind==="saga"?"Saga":kindLabel(n.kind);
    el.append(h("div",{class:"gk"},label));
    el.append(h("div",{class:"gt"},n.name));
    if(n.kind==="command"||n.kind==="event"||n.kind==="rejection"){
    ...
    el.onpointerdown=e=>{if(e.target.classList.contains("gport"))return;startMove(e,el,n.ref);};
    el.onclick=e=>{if(e.target.classList.contains("gport"))return;selectNode(n.id);};
    return el;
  }
