...
const SZ={HW:214,HHEAD:24,ROW:18,HPAD:8,GAPH:12,APAD:14,AHEAD:26,CPAD:18,CHEAD:32,GAPA:14,GAPC:44,LANEH:66,LANEGAP:90,LANEW:186,LANEGX:16,MAXROW:3000};
let board={};
...
function computeBoard(){
  ...
  const midTop=SZ.LANEH+SZ.LANEGAP;
  let cx=0,rowY=0,rowH=0,maxRight=0;
  measured.forEach(({c,m})=>{
    if(cx>0 && cx+m.w>SZ.MAXROW){ cx=0; rowY+=rowH+SZ.GAPC; rowH=0; }
    placeContext(c,m,cx,midTop+rowY);
    cx+=m.w+SZ.GAPC; rowH=Math.max(rowH,m.h); maxRight=Math.max(maxRight,cx-SZ.GAPC);
  });
  const contentBottom=midTop+rowY+rowH;
  const rightBound=Math.max(maxRight,SZ.LANEW*4);
...
function placeLane(items,y,rightBound){ let x=0,yy=y,rowH=SZ.LANEH;
  items.forEach(n=>{ if(x>0&&x+SZ.LANEW>rightBound){x=0;yy+=SZ.LANEH+SZ.LANEGX;}
    board.lane[n.id]={x,y:yy,w:SZ.LANEW,h:SZ.LANEH,node:n}; board.pick.push({x,y:yy,w:SZ.LANEW,h:SZ.LANEH,node:n}); x+=SZ.LANEW+SZ.LANEGX; }); }
...
function render(){
  ...
  Object.values(board.lane).forEach(b=>{ const n=b.node; const [x,y]=S(b.x,b.y); const act=simActive.has(n.id); const col=KIND_COL[n.kind];
    ...
    const icon=n.kind==='process'?'⬡ ':n.kind==='projection'?'▤ ':'⛁ '; ctx.fillText(icon+n.name, x+b.w*z/2, y+b.h*z/2-6*z);
    ctx.fillStyle=col; ctx.font=`${9.5*z}px -apple-system,sans-serif`;
    const sub=n.kind==='process'?n.process.pattern:(n.kind==='projection'?n.projection.subscriberId:'Pipeline');
    ctx.fillText(sub, x+b.w*z/2, y+b.h*z/2+9*z); ctx.textAlign='left'; ctx.globalAlpha=1; });
